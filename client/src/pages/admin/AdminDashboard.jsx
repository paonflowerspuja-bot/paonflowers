...
const AdminDashboard = () => {
  ...
  const loadOverviewData = async () => {
    ...
    try {
      ...
      try {
        ...
        mergedMetrics = {
          productsCount: m.productsCount ?? null,
          ordersCount: m.ordersCount ?? null,
          revenueToday: m.revenueToday ?? null,
      ...
      const ordersData = safeData(ordersRes);
      const productsData = safeData(productsRes);

      const recentOrdersList = Array.isArray(ordersData)
        ? ordersData
        : ordersData?.items || ordersData?.orders || [];

      const recentProductsList = Array.isArray(productsData)
        ? productsData
  ...
  // refresh overview when orders change
  useEffect(() => {
  ...
  // refresh overview when offers change
  useEffect(() => {
